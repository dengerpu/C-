...
//检查内存容量是否够
void checkCapicity(Students* p) {
	if (p->size == p->capicity) {
		Student* student = (Student*)realloc(p->data,DEFAULT_SIZE * sizeof(Student));
		if (student != NULL) {
			p->data = student;
			p->capicity = p->capicity + DEFAULT_SIZE;
			printf("增容成功\n");
		}
		else {
			printf("增容失败\n");
...
void InitStudent(Students* p) {
	...
	p->data = (Student *)malloc(DEFAULT_SIZE*sizeof(Student));
	if (p->data == NULL) {
		printf("%s\n", strerror(errno));
		return;
	}
	p->size = 0;
	p->capicity = DEFAULT_SIZE;
